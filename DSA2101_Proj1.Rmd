...
ggplot(top_overyears, aes(x = year, fill = spotify_genre)) +
  ...
  labs(title = "Density of Top 10 Genres Over the Years", 
       x = "Year", 
       y = "Density") +
...
ggplot(butterfly_data) + 
  geom_bar(aes(x = factor(features, level = features_by_desc_values), y = value, fill = type), 
           stat = 'identity') +
  scale_fill_manual(values = c("after_1991" = "#ccf2f3", "before_1991" = "#f7e9cc")) + 
  geom_text(data = subset(butterfly_data, type == "after_1991"), 
            aes(x = features, 
                y = value, 
                ...
                vjust = .5, 
                hjust = -0.3) +
  geom_text(data = subset(butterfly_data, type == "before_1991"), 
...
ggplot(unique_songs_df) +
  ...
  scale_color_gradient(low = "#80b1d3", 
                       high = "#3a4180") +
  scale_y_continuous(breaks= seq(15,30,by=1)) +
  geom_hline(yintercept = mean_abs_change, 
             linetype = 2) +
  annotate('rect', 
           xmin = 0.3, 
           xmax = 12.7, 
