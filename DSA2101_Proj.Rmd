...
# Xinyi Plots
## "Average Songs a Billboard Artist On Chart Each Year"

```{r}
billboard_song <- merged %>%
  separate(week_id, into = c("month", "day","year"), sep = "/") %>%
  select(year, song_billboard) %>%
  distinct() %>%
  group_by(year) %>%
  count() 
```
...
```{r}
ggplot(billboard_artist, aes(x=year,y=n)) +
  geom_col(fill = "lightblue") +
  scale_y_continuous(breaks =seq(0,800, by=100)) +
  scale_x_discrete(breaks =seq(1960,2020,by=10)) + 
  labs(title = "How Many Different Songs Make the Hot 100 Each Year?",
       y="Songs", x="Year") + 
  theme_minimal()
```

```{r}
billboard_chart = merged %>%
  separate(week_id, into = c("month", "day","year"), sep = "/") %>%
  filter(year!=2021) %>%
  select(year, song_billboard, performer) %>%
  distinct() 
```

```{r}
song_per_artist = billboard_chart %>%
  group_by(year) %>%
  summarise(song_per_artist=n()/ n_distinct(performer), .groups='drop')
```

```{r}
ggplot(song_per_artist, aes(x=year, y=song_per_artist)) +
  geom_col() +
  scale_y_continuous(breaks =seq(0,2.5, by=0.5)) +
  scale_x_discrete(breaks =seq(1960,2020,by=10)) + 
  labs(title = "Average Songs a Billboard Artist On Chart Each Year",
       y="Songs (Mean)", x="Year") + 
  theme_minimal()
```

## "Mean of Varience in 2010 to 2020, by Month"
...
ten_plot = top_overyears %>%
  ggplot(aes(x = year, y = n_songs, fill = spotify_genre)) +
...
## artist df cleaning
```{r}
artist_df = billboard_clean %>%
  select(week_id, song, performer) %>%
  unique() %>%
  mutate(year = year(mdy(week_id)))
```

## time series of top 10 artists of all time
```{r}
top_artist = artist_df %>%
  group_by(performer) %>%
  summarize(n_presence = n()) %>%
  mutate(rank = rank(desc(n_presence))) %>%
  filter(rank <= 10) %>%
  pull(performer)

artist_overyears = artist_df %>%
  group_by(year, performer) %>%
  summarize(n_presence = n()) %>%
  mutate(rank = rank(desc(n_presence))) %>%
  filter(performer %in% top_artist, rank <= 20) %>%
  arrange(year, rank)
  
artist_plot = artist_overyears %>%
  ggplot(aes(x = year, y = rank, group = performer, color = performer)) +
  geom_point() +
  geom_line() +
  scale_y_reverse()
